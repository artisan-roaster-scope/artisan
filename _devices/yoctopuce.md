...
- `Yocto Power`:
  ...
  - channel 2: reports the integrated power consumption over time (`meter`). Note that this counter is reset at each start of the device or via the `Artisan IO Command` `powerReset([sn])`.

- `Yocto Energy`:
  - channel 1: reports the delivered energy (`deliveredEnergyMeter`), maintained by integrating the power consumption over time when positive only. Note that this counter is reset at each start of the device or via the `Artisan IO Command` `powerReset([sn])`.
  - channel 2: reports the received energy (`receivedEnergyMeter `), maintained by integrating the power consumption over time when negative only. Note that this counter is reset at each start of the device or via the `Artisan IO Command` `powerReset([sn])`.
