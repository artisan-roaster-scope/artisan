...
Note: to read additional channels, add corresponding extra devices using the device types `WebSocket 34`, `WebSocket 56`, `WebSocket 78` and `WebSocket 910`, each reading another two channels
...
using `command` as Command node, `id` as Message ID node, `getData`the as Data Request node and `machine` as Machine ID node. As explained above, the machine id is taken from the ID field and the message id is automatically generated by Artisan per request.

The lower section specifies the tags delivering the actual data per input channel. 
The Data node specifies that all data is delivered in a node named `data`. The BT, Input 1, is taken from the (sub-)node `BT` and the ET, Input 2, from the (sub-)node `ET` in this example. Thus the response on the above request should look like this
...
Instead of harvesting all input channels efficiently with one request, it is also possible to configure Artisan to request data per input channel individually by leaving the `getData` node empty and specifying Request nodes per individual channel, in this example using the nodes `getBT` and `getET`.
