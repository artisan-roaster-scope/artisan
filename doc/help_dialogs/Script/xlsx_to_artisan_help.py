...
If filename is not specified 'help_pages' will be used.
...
Excel file format:
    ...
    Cells formatted as Italic or with any font color will not be sent to QApplication.translate().
    
    Use only double quotes in cell values, no single quotes.
...
def getTitle(all_rows,ws,nsheet):
    if nsheet == 0:
        title = '\n' + 'helpstr += "<b>" + ' + str(all_rows[0][0]) + ' + "</b>"'
    else:
        title = '\n' + 'helpstr += "<br/><br/><b>" + ' + str(all_rows[0][0]) + ' + "</b>"'
    return title
...
def getNotes(all_rows,nrows,tbl_name,notetype='top'):
    ...
    tbl_notes += '\n' + tbl_name + ' = prettytable.PrettyTable()'
    tbl_notes += '\n' + tbl_name + '.header = False'
    tbl_notes += '\n' + tbl_name + '.add_row([' + notes + '])'
...
def buildpyCode(fname_in,fname_out_html):
    ...
    for ws in wb.worksheets:
        ...
        if nrows_topnotes > 0:
            outstr += tbl_topnotes
            outstr += '\n' + 'helpstr += ' + tbl_name + 'top' + '.get_html_string(attributes={' + note_table_attributes + '})'
        ...
        if len(tbl_addrows) > 0:
            ...
            outstr += '\n' + tbl_fieldnames
            outstr += '\n' + tbl_addrows
            outstr += '\n' + 'helpstr += ' + tbl_name + '.get_html_string(attributes={' + data_table_attributes + '})'

        # add bottom notes
        if nrows_bottomnotes > 0:
            outstr += tbl_bottomnotes
            outstr += '\n' + 'helpstr += ' + tbl_name + 'bottom' + '.get_html_string(attributes={' + note_table_attributes + '})'
...
#        fname_out_html = join(dirname(abspath(__file__)), 'output_files', sys.argv[1] + '.html')
    if len(sys.argv) > 1:
        ...
        fname_in = '../input_files/help_pages.xlsx'
        fname_out = '../output_files/help_pages.py'
        fname_out_html = '../output_files/help_pages.html'
