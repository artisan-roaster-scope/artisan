...
加载在较小机器上记录的配置文件并打开 Transposer。选择线性映射并将在大型机器上观察到的 DRY 和 FC START 目标温度放入 BT 下的相应字段中。在表格下方，您会看到计算出的符号公式，可以将其复制粘贴到 Config &gt;&gt; Devices 下的 BT 符号公式中，以便在较小的机器上烘烤时自动调整计算的映射，以在较大的机器上查看您所期望的温度读数.</translation>
    ...
    <message>
      ...
      <translation>注意：占位符 {ET}、{BT}、{time}、{ETB}、{BTB}、{WEIGHTin} 和 {TEMP} 将被替换为当前 ET、BT、时间、ET 背景、BT 背景值、批次大小（以克为单位）、以 F 为单位的事件值（在 Serial/Artisan/CallProgram/MODBUS/S7/WebSocket 命令中转换为当前温度单位）
</translation>
    </message>
    <message>
      <location filename="../help/eventsliders_help.py" line="34"/>
