...
- Comprehensive web scraping and data parsing validation
...
class TestRoastLogDataParsing:
    """Test RoastLog data parsing functionality without complex dependencies."""
    ...
    def test_weight_parsing_logic(self) -> None:
        ...
        def parse_weight_data(starting_mass: str, ending_mass: str) -> List[Any]:
            """Local implementation of weight parsing for testing."""
            w_in = '0'
            w_out = '0'
            u = 'lb'

            if starting_mass:
                w_in, u = starting_mass.strip().split(' ')
            if ending_mass:
                w_out, u = ending_mass.strip().split(' ')

            return [float(w_in), float(w_out), ('Kg' if u.strip() == 'kg' else 'lb')]

        # Test with lb units
        result_lb = parse_weight_data('140.00 lb', '116.80 lb')
        assert result_lb == [140.0, 116.8, 'lb']

        # Test with kg units
        result_kg = parse_weight_data('63.5 kg', '53.0 kg')
        assert result_kg == [63.5, 53.0, 'Kg']

        # Test with zero values
        result_zero = parse_weight_data('0 lb', '0 lb')
        assert result_zero == [0.0, 0.0, 'lb']
