...
#    currentServiceChanged = QtCore.pyqtSignal(int)
    ...
    def __init__(self, service_uuid, char_uuid, processData, sendHeartbeat, reset):
        ...
        self.m_control = None # : QtBluetooth.QLowEnergyController
        #self.m_servicesUuid = QtBluetooth.QBluetoothUuid()
        self.m_servicesUuid = []
        self.m_service = None # : QtBluetooth.QLowEnergyService
...
#        self.statusInfoChanged.emit("Device disconnected",False)
        ...
        if self.m_service is not None:
            ...
            self.m_service.characteristicWritten.disconnect()
#            self.m_service.error.disconnect()
...
#    @QtCore.pyqtSlot("QBluetoothUuid")
#    def onServiceDiscovered(self,_):
...
#                print("service characteristic not found")
    ...
    def disconnectDevice(self):
        if self.m_readTimer is not None:
            self.m_readTimer.timeout.disconnect()
            self.m_readTimer.stop()
...
#    @QtCore.pyqtSlot("QLowEnergyController::Error")
