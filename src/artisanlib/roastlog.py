...
def extractProfileRoastLog(url,_):
    ...
    try:
        ...
        for tag in ["Roastable:", "Starting mass:", "Ending mass:", "Roasted on:", "Roasted by:", "Roaster:", "Roast level:", "Roast Notes:"]:
            tag_elements = tree.xpath('//td[contains(@class,"text-rt") and normalize-space(text())="{}"]/following::td[1]/text()'.format(tag))
        ...
        if len(source_elements)>0:
            ...
            if len(d)>0:
                ...
                url = "https://roastlog.com/roasts/profiles/?rid={}".format(rid)
                headers = {
