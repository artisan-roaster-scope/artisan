...
def content():
    ...
    tbl_Protocol.add_row([QApplication.translate('HelpDlg','Typical number of batches per session',None),QApplication.translate('HelpDlg','This value is used to allocate the preheating energy equally across the number of batches in the roasting session.',None)])
    tbl_Protocol.add_row([QApplication.translate('HelpDlg','Auto',None),QApplication.translate('HelpDlg','Requires Artisan plus.  When checked, Artisan will make an estimate of the number of batches per roast session.',None)])
