...
<head>
    ...
    <script type="text/javascript">
        ...
        let BUCKETPOSITION = 'calc(12% - 0.5vmin)';
        ...
        function getAllElements() {
            for (const prop of ['id', 'batchsize', 'scale_rect',
                'percent', 'scale_icon_done', 'scale_icon_initial',
                'blend_percent',
        ...
        function setInitialStyles() {
            document.getElementById('html').style.backgroundColor = BACKGROUND;
            ...
            document.getElementById('buckets_grid_part').style.stroke = BUCKETCOLOR;
        }

        function setTexts() {
            for (const prop of ['id', 'batchsize', 'weight', 'blend_percent']) {
                elements[prop].textContent = data[prop] || '';
            }
            for (const prop of ['title', 'subtitle',]) {
                // there are 2 of each (only one ever displayed)
                elements[prop].forEach(el => el.textContent = data[prop] || '');
            }
            for (const prop of ['percent']) {
                if (Number.isFinite(data[prop])) {
                    elements[prop].textContent = Math.round(data[prop]) + '%';
                } else {
        ...
        usedata = function (evt) {
            ...
            if (data) {
                ...
                for (let i = 0; i < elements.buckets_images.length; i++) {
                    ...
                    if (el && el.style) {
                        el.style.display = i < (data.bucket || 0) ? 'block' : 'none';
                    }
                }
                ...
                switch (data.state) {
                    ...
                    case DONE:
                        ...
                        if (!data.percent) {
                            ...
                            elements.bucket_on_scale.style.top = 'calc(12% - 1.2vmin)';
                            elements.bucket_on_scale.style.left = 'calc(12% - 1.2vmin)';
                    ...
                    case WEIGHING:
                        ...
                        if (data.percent >= 0) {
                            ...
                            elements.percent.innerHTML = data.percent.toFixed(0) + '%';

                            if (data.percent > 0) {
                                if (data.percent <= 100) {
                                    elements.scale_rect.style.background = 'linear-gradient(0deg, #2098c7 0 ' + data.percent + '%, #b5b5b5 ' + data.percent + '% 100%)';
                                    if (data.percent === 100) {
                                        elements.bucket_on_scale.style.color = '#b5b5b5';
                                        elements.bucket_on_scale.style.backgroundColor = '#2098c7';
                                        elements.percent.style.color = 'white';
                                    }
                                }
                            }
                ...
                //             totalScaleEl.style.background = 'linear-gradient(0deg, #2098c7 0 ' + data.total_percent + '%, #b5b5b5 ' + data.total_percent + '% 100%)';
                //             if (data.total_percent === 100) {
                //                 totalPercentEl.style.color = '#b5b5b5';
                //             }
                //         } else {
                //             // overlap
                //             totalPercentEl.style.color = '#b5b5b5';
        ...
        function setColorScheme(scheme) {
            switch (scheme) {
                ...
                default:
                    ...
                    BUCKETPOSITION = 'calc(12% - 0.5vmin)';
                    break;
            }
            setInitialStyles();
            if (lastdata) {
    ...
    <style type='text/css'>
        ...
        @media (max-aspect-ratio: 11/10) {
            /* .scale-rect,
               .scale-icon {
                max-height: calc(100vh - 2*(16px + max(40px, min(7vh, 7vw)) + 60px + max(32px, min(6vh, 6vw))) - 10px);
            } */
            ...
            <div class="scale-and-buckets" id="scale_and_buckets">
                <div class="scale-div">
                    <div class="scale-rect" id="scale_rect">
                        <!-- <div class="scale-rect_overflow" id="scale_rect_overflow"></div> -->
