...
const calculateStyles = (f) => {
    ...
    f.currentFontSize = Math.min(
        ...
        f.maxSize,
        maxHeightElement ? (Number.parseFloat(w.getComputedStyle(maxHeightElement).fontSize) || f.maxSize) : Infinity,
    );
...
function fitty(target, options = {}, maxHeightEl = undefined) {
    maxHeightElement = maxHeightEl;
