...
def addSyncShelve(uuid: str, modified_at, fh):
    ...
    try:
        with shelve.open(getSyncPath()) as db:
            db[uuid] = modified_at
        config.logger.debug(
            "sync: DB type: %s", str(dbm.whichdb(getSyncPath()))
        )
    except Exception as e:  # pylint: disable=broad-except
        _, _, exc_tb = sys.exc_info()
        config.logger.error(
            ("sync: Exception in addSyncShelve(%s,%s) line:"
             " %s shelve.open (1) %s"),
            str(uuid),
            str(modified_at),
...
def addSync(uuid, modified_at):
    try:
        ...
        _, _, exc_tb = sys.exc_info()
        config.logger.info(
            "sync: LockException in addSync(%s,%s) line: %s %s",
            str(uuid),
            str(modified_at),
            getattr(exc_tb, 'tb_lineno', '?'),
            e,
        ...
        try:
            ...
            _, _, exc_tb = sys.exc_info()
            config.logger.error(
                "sync: Exception in addSync(%s,%s) line: %s %s",
                str(uuid),
                uuid(modified_at),
                getattr(exc_tb, 'tb_lineno', '?'),
                e,
...
def getSync(uuid):
    try:
        ...
        with portalocker.Lock(getSyncPath(lock=True), timeout=0.5) as fh:
            try:
                with shelve.open(getSyncPath()) as db:
                    try:
                        ts = db[uuid]
                        config.logger.debug(" -> sync:getSync = %s", ts)
                        return ts
                ...
                _, _, exc_tb = sys.exc_info()
                config.logger.error(
                    "sync: Exception in getSync(%s) line: %s shelve.open %s",
                    str(uuid),
                    exc_tb.tb_lineno,
                    e,
                )
        ...
        config.logger.info(
            "sync: LockException in getSync(%s) line: %s %s",
            str(uuid),
            getattr(exc_tb, 'tb_lineno', '?'),
            e,
        )
        ...
        try:
            ...
            with portalocker.Lock(getSyncPath(lock=True), timeout=0.3) as fh:
                try:
                    ...
                    _, _, exc_tb = sys.exc_info()
                    config.logger.error(
                        ("sync: Exception in getSync(%s) line:"
                         " %s shelve.open %s"),
                        str(uuid),
                        getattr(exc_tb, 'tb_lineno', '?'),
                        e,
            ...
            _, _, exc_tb = sys.exc_info()
            config.logger.error(
                "sync: Exception in getSync(%s) line: %s %s",
                str(uuid),
                getattr(exc_tb, 'tb_lineno', '?'),
                e,
            )
    ...
    try:
        ...
        with portalocker.Lock(getSyncPath(lock=True), timeout=0.5) as fh:
            try:
                ...
                _, _, exc_tb = sys.exc_info()
                config.logger.debug(
                    "sync: Exception in delSync(%s) line: %s KeyError %s",
                    str(uuid),
                    getattr(exc_tb, 'tb_lineno', '?'),
                    e,
                )
                ...
                _, _, exc_tb = sys.exc_info()
                config.logger.error(
                    "sync: Exception in delSync(%s) line: %s shelve.open %s",
                    str(uuid),
                    getattr(exc_tb, 'tb_lineno', '?'),
                    e,
                )
        ...
        config.logger.info(
            "sync: LockException in delSync(%s) line: %s %s",
            str(uuid),
            getattr(exc_tb, 'tb_lineno', '?'),
            e,
        )
        ...
        try:
            ...
            with portalocker.Lock(getSyncPath(lock=True), timeout=0.3) as fh:
                try:
                    ...
                    _, _, exc_tb = sys.exc_info()
                    config.logger.error(
                        ("sync: Exception in delSync(%s) line:"
                         " %s shelve.open %s"),
                        str(uuid),
                        getattr(exc_tb, 'tb_lineno', '?'),
                        e,
            ...
            _, _, exc_tb = sys.exc_info()
            config.logger.debug(
                "sync: Exception in delSync(%s) line: %s %s",
                str(uuid),
                getattr(exc_tb, 'tb_lineno', '?'),
                e,
            )
...
def setApplidedServerUpdatesModifiedAt(modified_at):
    ...
    try:
        ...
        _, _, exc_tb = sys.exc_info()
        config.logger.error(
            ("roast: Exception in setApplidedServerUpdatesModifiedAt()"
             " line %s: %s"),
            getattr(exc_tb, 'tb_lineno', '?'),
            e,
        )
...
def getApplidedServerUpdatesModifiedAt():
    ...
    try:
        ...
        _, _, exc_tb = sys.exc_info()
        config.logger.error(
            ("roast: Exception in clearApplidedServerUpdatesModifiedAt()"
             " line %s: %s"),
            getattr(exc_tb, 'tb_lineno', '?'),
            e,
        )
...
def fetchServerUpdate(uuid: str, file=None):
    ...
    try:
        config.logger.debug(
            ("sync:fetchServerUpdate() -> requesting update"
             " from server (file: %s)"),
            file,
        )
        last_modified = ""
        ...
        if status == 204:  # NO CONTENT: data on server is older then ours
            ...
            if file is not None and getSync(uuid) is None:
                config.logger.debug(
                    ("sync: -> file not in sync cache yet, we need to fetch"
            ...
            if "result" in data:
                ...
                if file_last_modified is not None:
                    config.logger.debug(
                        "sync: -> file last_modified date: %s",
        ...
        config.logger.debug(
            ("sync: -> connection error in fetchServerUpdate(),"
             " disconnecting: %s"),
            e,
        )
...
def getUpdate(uuid: str, file=None):
    config.logger.info("sync:getUpdate(%s,%s)", uuid, file)
    if uuid is not None:
        aw = config.app_window
...
def sync():
    try:
        ...
        _, _, exc_tb = sys.exc_info()
        config.logger.error(
            "sync: Exception in sync() line %s: %s", exc_tb.tb_lineno, e
        )
